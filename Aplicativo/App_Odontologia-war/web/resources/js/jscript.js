...
function AbrirSeguro(path)
{
    ...
    window.open("https://www.snna.gob.ec/aspirante", "Registro Datos", cadena);
}
...
function fecha() {
    Stamp = new Date();
    dia = Stamp.getDay() + 1;
    num = ((Stamp.getDate() < 10) ? "0" : "") + Stamp.getDate();
    ...
    Hoy = (Dia + " " + num + " de " + Mes + " de " + year + "  " + Hours + ":" + Mins + Time);
    return Hoy;
// FIN -->
}
...
function imprSelec(nombre)
{
    ...
    if (navigator.userAgent.indexOf('Opera') >= 0) {
        ...
        var cadena;
        var ancho = screen.availWidth - 30;
        var alto = screen.availHeight - 100;
        cadena = 'left=0,top=0,border=0,status=no,scrollbars=yes,toolbar=no,menubar=no,location=yes,resizable=yes,directories=no,width=' + ancho + ',height=' + alto;
        var ficha = document.getElementById(nombre).innerHTML;
        var ventimp = window.open('/aspirante/blanco.xhtml', 'popimpr', cadena);
        ventimp.document.write('<html><head><title>SNNA</title>');
        ventimp.document.write('<link href="resources/css/sistema.css" type="text/css" rel="stylesheet"/>');
        ventimp.document.write('<script src="resources/js/jscript.js"></script>');
        ventimp.document.write('<script> $(document).ready(function() {$("#cabeceraReporte").show(); $("#imp1").show(); $("#imp2").hide(); $("#tablaDatos").width("90%");});</script>');
        ventimp.document.write('</head><body >');
        ventimp.document.write('<table bgcolor="White"> <tr> <td colspan=2>')
        ventimp.document.write(ficha);
        ventimp.document.write('</td> </tr> ')
        ventimp.document.write('<tr class="franjaBlanca"> <td class="tituloBold"> Fecha de Impresión: </td>   <td class="titulo"> ');
        ventimp.document.write(fecha());
...
function ValidaInputsNumericos()
{
    $('input[alt="numeros"]').numeric();

    $("input[id=iceFormRight\\:txtHojasUtilizadasPerso]").blur(function()
    {
        var numeroTotal = parseInt($("#totalMonitoreo").html());
        var valorActual = parseInt($(this).val());

        if (numeroTotal != valorActual)
        {
            var balanceIzquierda = parseInt(20);
            var position = $(this).position();
            var height = $(this).height();
            var width = $(this).width();
            var top = position.top - height + "px";
            var right = position.left + width + balanceIzquierda + "px";
            ...
            $('#tooltip').show();
            $(this).css("box-shadow", "0 0 3px #FF0000").css("border-color", "#D64242");
        }
        else
        {
            $('#tooltip').hide();
            $(this).css("box-shadow", "").css("border-color", "");
        }
    });
}
...
(function(window, undefined) {
    ...
    function _getMarkup(element, opts) {
        ...
        //
        //
        html.push('<script src="resources/js/jscript.js"></script>');
        html.push('<script> $(document).ready(function() {$("#cabeceraReporte").show(); $("#imp1").show(); $("#imp2").hide(); $("#tablaDatos").width("90%");});</script>');
        // ventimp.document.write('<script> $(document).ready(function() {$("#cabeceraReporte").show(); $("#imp1").show(); $("#imp2").hide(); $("#tablaDatos").width("90%");});</script>');             
...
function imprime()
{
    $("#content3").printElement({
        leaveOpen: false,
        printMode: 'popup',
        overrideElementCSS: ['resources/css/sistema.css']
    });
}